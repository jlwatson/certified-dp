...
fn sample_prover_step(state: &ProverState,
    prover_mailbox: &mut VecDeque<String>, verifier_mailbox: &mut VecDeque<String>) {
    
}

fn sample_verifier_step(state: &VerifierState,
    prover_mailbox: &mut VecDeque<String>, verifier_mailbox: &mut VecDeque<String>) {
...
fn commitment_phase<R: Rng + CryptoRng, T: PrimInt + Hash>(rng: &mut R, pedersen_pp: &PublicParams) -> HashMap<T, (Scalar, RistrettoPoint, Scalar)> {
...
fn main() {
    ...
    let json_message = serde_json::to_string(&test_message).unwrap();

    println!("JSON message: {}", json_message);

    let deserialized_message: TestMessage = serde_json::from_str(&json_message).unwrap();
